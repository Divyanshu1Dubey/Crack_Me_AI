...
def test_api_keys():
    ...
    def _test_openai_compat(name, env_var, base_url, model, timeout=15.0):
        ...
        try:
            ...
            if '429' in err:
                ...
                fail(name, "API key INVALID — get new key")
    ...
    if key:
        try:
            ...
            err = str(e)
            if '429' in err:
                skip("Groq", "Rate limited")
            elif '401' in err:
                fail("Groq", "API key invalid")
            else:
                fail("Groq", err[:100])
    else:
    ...
    if key:
        try:
            ...
            err = str(e)
            if '429' in err:
                skip("Cerebras", "Rate limited")
            elif '401' in err:
                fail("Cerebras", "API key invalid")
            else:
                fail("Cerebras", err[:100])
    else:
    ...
    if key:
        try:
            ...
            if '429' in err or 'RESOURCE_EXHAUSTED' in err:
                skip("Gemini", "Quota exhausted")
            elif '401' in err or '403' in err:
                fail("Gemini", "API key invalid")
            else:
                fail("Gemini", err[:100])
    else:
    ...
    if key:
        try:
            ...
            err = str(e)
            if '429' in err:
                skip("Cohere", "Rate limited")
            elif '401' in err:
                fail("Cohere", "API key invalid")
            else:
                fail("Cohere", err[:100])
    else:
...
def test_config():
    ...
    if reqs.exists():
        lines = [line.strip() for line in reqs.read_text().splitlines() if line.strip() and not line.startswith('#')]
