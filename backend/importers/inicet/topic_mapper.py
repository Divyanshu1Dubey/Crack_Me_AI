...
def map_subject(stem: str, fallback: Optional[str] = None) -> Optional[str]:
    """Subject classifier — filename fallback wins for INI-CET.

    The INI-CET material PDFs are reliably named "<Subject> pyqs.pdf"
    (e.g. "Anatomy pyqs.pdf", "Surgery pyqs.pdf"). The runner passes the
    filename-derived subject as `fallback`, so we prefer it over keyword
    scoring when scoring is zero. When the stem contains strong keyword
    hints for a *different* subject, those win.
    """
    text = (stem or "").lower()
    ...
    for subject, kws in SUBJECT_KEYWORDS.items():
        ...
        if score > best[0]:
            best = (score, subject)
    # Keyword match wins when it has at least one hit.
    if best[0] >= 1 and best[1]:
        return best[1]
    # Otherwise trust the filename (which is reliable for INI-CET).
    return fallback
