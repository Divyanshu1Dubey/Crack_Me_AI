...
const BACKEND_API_BASE_URL = normalizeApiBaseUrl(
  (process.env.BACKEND_API_URL || process.env.NEXT_PUBLIC_API_URL || DEFAULT_BACKEND_API_URL).trim()
);
...
const buildTargetUrl = (request: NextRequest, pathParts: string[]) => {
  const path = pathParts.join('/');
  const target = `${BACKEND_API_BASE_URL}/${path}`;
